// Indoor Nav — Advanced (touch + A* routing + animated runner)
...
let routePolyline=[], instructions=[];
...
fetch('building.geojson').then(r=>r.json()).then(async graph => {
  ...
  resetBtn.onclick = ()=>{ anim.t=0; anim.playing=false; renderInstructions(); render(); };

  // Space toggles play/pause
  window.addEventListener('keydown', (e)=>{ if (e.code==='Space'){ anim.playing=!anim.playing; e.preventDefault(); } });
...
function renderInstructions(){
  ...
  const totalSteps = Math.round(totalPx / PX_PER_STEP);
  etaBadge.textContent = `ETA ~${Math.max(1, Math.round((totalPx/anim.speed)/60))} min, ${totalSteps} steps`;

  // Simple textual instructions (straight/vertical)
  const ids = routePolyline.map(p=>p.id);
  for (let i=1;i<ids.length;i++){
    ...
    if (vertical){
      ...
      li.textContent = `Take the ${via} ${dir} to Floor ${B.floor}`;
    } else {
      li.textContent = `Go ${steps} steps straight`;
    }
...
function totalLengthPx(poly){
  let L=0; for (let i=1;i<poly.length;i++){ const a=nodesById.get(poly[i-1].id), b=nodesById.get(poly[i].id); L+=Math.hypot(b.x-a.x, b.y-a.y); } return L;
}

// Animation loop
function tick(ts){
  if (anim.playing && routePolyline.length){
    const dt = (tick.lastTs ? (ts - tick.lastTs)/1000 : 0);
    anim.t += anim.speed * dt;
    const L = totalLengthPx(routePolyline);
    if (anim.t >= L) { anim.t = L; anim.playing=false; }
    render();
  }
  tick.lastTs = ts;
